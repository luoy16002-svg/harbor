...
Cards are checked from top to bottom. Move them with the arrow buttons, edit, disable, or remove them. Domain and process entries within one card are alternatives: either can match. A disabled card retains its settings. Cards and the editor distinguish fully covered matchers, partial domain/process overlap, and possible intersections between application and website conditions. Only enabled earlier paths affect this advice; it does not query the network or process owner. Unchanged status refreshes preserve the existing card controls and focus. Saving affects new TCP connections and new UDP destination sessions; already established sessions retain their previous configuration. To apply a stricter requirement to an existing session, close that session or disconnect before changing the requirement.
...
5. Automatic-group selection among members supporting the transport and satisfying both path and global restrictions.
6. Final checks for fixed or selected members, including transport support and global restrictions.
...
All automatic groups exclude members that cannot forward the requested transport, including HTTP/HTTPS for UDP. Global proxy-encryption and non-loopback DIRECT restrictions are applied before selection, as well as the individual path requirement and measured unavailability. Requiring global proxy encryption alone still permits an explicitly configured DIRECT member; an individual encrypted-proxy path excludes DIRECT, including loopback. Global DIRECT blocking retains its existing loopback exception. Unmeasured members can still be attempted; eligibility is not a successful health check. TCP and UDP eligibility are evaluated separately. A TLS-protected SOCKS5 TCP connection does not encrypt the standard SOCKS5 UDP relay. HTTP/HTTPS proxies have no UDP forwarding support. A protected or UDP request with no eligible member is rejected with a reason. Fixed HTTP/HTTPS UDP requests are also rejected during route evaluation, so the offline checker agrees with forwarding support. A manually selected group member is never silently replaced. Sticky state is separate for TCP/UDP and the effective encryption/direct restrictions, with at most eight entries per group. Switching one transport or protection class does not move another class. Capability counts on automatic-group cards use configuration and non-loopback destinations; they do not include measured health.
...
Policy checks cover global guards before selection, independent unprotected TCP/UDP choices, loopback exceptions, eligibility after edits, fixed-member rejection, ordered overlap, all modes, legacy exceptions, global blocks, per-transport protected groups, separate selection state, bounded configuration, and process rehearsal without network requests. Real Windows process fixtures exercise HTTP CONNECT, SOCKS TCP and SOCKS UDP forwarding with the new paths enabled and legacy exceptions disabled; old connections retain their generation after a path is disabled.

Two independent Xray checks transfer real TCP and UDP payloads through automatic groups, including global encryption and plaintext/HTTPS members that must be skipped. Desktop checks cover priority advice, capability counts, preserved card focus, path-only subscription references, encrypted history restoration, capability descriptions and verification fingerprints. Isolated WPF checks use real editor and card buttons, invalid input, priority changes, enable/disable/remove, process previews, node renaming, and forced atomic-save failure. Screenshots contain local fixture names, not customer traffic or claimed internet performance. Live game capture, TUN recovery and reputation assessments remain unverified. See [verification](verification.md).
