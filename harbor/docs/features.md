...
Implemented features and the checks completed for 0.4.0 are listed separately from unverified behavior.
...
| Proxy search and saved checks | Name, protocol, and server search; encrypted results; expiry after configuration changes or 24 hours | Persistence, filtering, clearing, per-proxy invalidation, and timestamp checks |
...
| HTTPS verification | Proxy handshake, target certificate validation, and an HTTPS HEAD request | Success and failure fixtures, timeout, and cancellation |
