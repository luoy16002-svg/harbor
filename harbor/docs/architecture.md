...
The optional `routingMode` configuration field defaults to `rules` for existing workspaces. A local domain block is evaluated first, then the first matching enabled `trafficRoutes` entry, then enabled domain/process `directExceptions` if no path matched. An exception requests `DIRECT`; otherwise the mode is evaluated. In rules mode, the first matching enabled rule or `finalPolicy` chooses a policy. In `global` mode, `finalPolicy` chooses it. In `direct` mode, the policy is `DIRECT`. Group selection and transport privacy rejection follow. A path can reference a terminal outbound, a group, or `finalPolicy`; it overrides every routing mode. Its encryption requirement excludes DIRECT and non-encrypted automatic-group members per transport. Automatic selection also filters global encryption/direct restrictions and transport support. TCP/UDP and effective encryption/direct flags key at most eight sticky selections per group; fixed members are not silently replaced. See [traffic paths](traffic-paths.md) for bounds and rejection behavior. Non-rule modes retain and validate the saved rules and references. The snapshot exposes the active mode.

Windows process paths and exceptions use read-only endpoint ownership tables and limited image queries. TCP uses a full connection tuple; UDP uses the datagram binding, refusing conflicting owners. Source context comes from accepted sockets or packet endpoints, never from an asserted process name in proxy input. Four bounded blocking jobs serve new flows with a 250 ms caller deadline; timed-out work holds its permit until completion. There is no PID or executable cache. Missing ownership continues through domain and default routing. A lookup is skipped only when no preceding process path can change the first matching domain path. See [direct exceptions](direct-exceptions.md) for precise bounds, capture limitations and tests.
...
An optional `pool` object on a fallback/latency group enables bounded setup recovery and, when enabled, background HTTPS checks. The selector keeps at most 256 member-health records across eight pools. Two monitor jobs share the engine resolver and verified destination TLS implementation; generation and physical-interface epochs cancel obsolete work. Fingerprints invalidate health after credential, DNS, trust, check-target or protection changes. Probe snapshots expose categorized outcomes, never the configured check URL or detailed errors.

All TCP ingress uses `Engine::connect_flow`. The policy decision carries the effective encryption requirement into retry selection. Only eligible concrete pool members can be attempted, at most three times within one global deadline, before application payload forwarding. Registered streams retain cancellation handles until dropped, including setup/upgrade failures. No application replay or UDP retry is introduced. Flow telemetry records attempts and the winning outbound; recent pool metadata obeys hiding, retention and clearing. See [automatic pools](automatic-pools.md) for health thresholds and selection behavior.
...
The rehearsal IPC validates two candidate configurations and compares target decisions using fresh selectors, without DNS, health probes or listeners. An optional process filename simulates ownership only in this offline command; forwarding derives ownership from actual endpoints. Its response explicitly reports `offline`, `networkRequests: 0` and `healthMeasured: false`. The UI's candidate editor exposes routing/privacy fields without exposing node credentials.
